...
public class CreateNewPost extends AppCompatActivity implements View.OnClickListener, Invoker {
    ...
    private void createNewPost() {
        ...
        pd = ProgressDialog.show(this, "Uploading Shayri", "Uploading new Shayri, please wait...");

        ce.execute(command);

        Bundle bundle = new Bundle();
        bundle.putString("action", "post_upload");
        Application.getApplicationInstance().getFirebaseAnalytics().logEvent("shayri_upload", bundle);
    ...
    public void NotifyCommandExecuted(ResultObject result) {
        ...
        if(result.isCommandExecutionSuccess() && result.getResultCode() == 200){
            Toast.makeText(this, "Shayari uploaded successfully...", Toast.LENGTH_LONG).show();
            showInterstitialAd();
            finish();
        }
        else{
            String message = "Following error while uploading Shayari: " + result.getErrorMessage() + " -- Please try again!";
